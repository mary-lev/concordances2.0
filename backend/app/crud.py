...
def get_location(db: Session, location_id: int) -> schemas.LocationInDBBase:
    return db.query(models.Location.name).filter(models.Location.id == location_id).first()

def get_location_by_name(db: Session, name: str) -> schemas.LocationInDBBase:
...
def get_text(db: Session, id: int) -> models.Text:
...
def get_new_text_id_by_old_id(db: Session, old_id: int) -> int:
...
def get_variant(db: Session, id) -> models.Variant:
...
def get_old(db: Session, id) -> models.Old:
    return db.query(models.Old).filter(models.Old.id == id).first()

def get_old_for_text(db: Session, text_id: int) -> models.Old:
