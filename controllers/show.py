...
def show1():   # show text from file
    ...
    epi_text=[]
    if texts.epigraph:
        epig = trymysql(trymysql.epi.text==request.args(0)).select()
        for all in epig:
            with open(all.epi_filename, 'r') as f:
                epi_lines = f.readlines()
                epi_text.append(epi_lines)
            epi.append(all)

    return dict(texts=texts, content=content, image=image, date=date, epigraphs=epi, epi_text=epi_text)
