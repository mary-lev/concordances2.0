...
def tokenize_all(): # prepares text for tokenization (decoding) and write result in database trymysql.allword, after - morpho/index1.html
    for x in range(5700,5881): # if text not yet in database (last:4811)
        text2 = trymysql(trymysql.text1.id==x).select().first()
        text1 = text2['body'].decode('utf-8')
        path = "/home/concordance/web2py/applications/test/uploads/balmont/"
...
def search_for_files():
    ...
    for all in content:
        ...
        if 'NNN' in all:
            ...
            text['t']=stih
            poems.append(text)
            trymysql.text1.insert(title=zagl, first_string=stih[0]+str("..."), year_writing=year, epigraph = epi, epigraph_author = a_epi, dedication= ded, month_writing=month, day_writing=day, author=14, writing_location=location, body = ''.join(text['t']))
            stih = []
            text= {}
            year=''
            zagl=''
