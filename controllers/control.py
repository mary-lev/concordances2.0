...
def tokenize_all(): # prepares text for tokenization (decoding) and write result in database trymysql.allword, after - morpho/index1.html
    for x in range(10283,10453): # if text not yet in database (last:4811,7088, 10260)
...
def search_for_files():
    ...
    for all in content:
        ...
        if 'NNN' in all:
            ...
            text['t']=stih
            poems.append(text)
            trymysql.text1.insert(title=zagl, first_string=stih[0]+str("..."), year_writing=year, epigraph = epi, epigraph_author = a_epi, dedication= ded, month_writing=month, day_writing=day, author=24, group_text=82, writing_location=location, body = ''.join(text['t']))
            stih = []
            text= {}
            year=''
            zagl=''
